...
{
    public function user_mon_compte(){
        $user = Auth::user();
        return view('comptes.user_mon_compte',['user'=>$user]);
    }

    public function user_edit_informations_form(){
        $user = Auth::user();
        return view('comptes.user_edit_informations_form',['user'=>$user]);
    }

    public function user_edit_informations(Request $request){
        $request->validate([
        ...
        return redirect()->route('user.page_mon_compte')->with('etat','Modification des infos réussi !');        
    }

    public function user_change_mdp_form(){
        return view('comptes.user_change_mdp_form');
    }

    public function user_change_mdp(Request $request){
        $request->validate([
        ...
        return redirect()->route('user.change_mdp_form')->with('etat','Il semble que l\'ancien mot de passe ne soit pas bon');
    }

    public function enseignant_liste_cours_associer($id){
    ...
    public function enseignant_liste_seances_cours($cid,$eid){
        $cours = Cour::findOrFail($cid);
    ...
    public function enseignant_liste_etudiant_seance($cid,$sid,$eid){
        $cours = Cour::findOrFail($cid);
        $liste_etudiants = $cours->etudiants;
    ...
    public function enseignant_liste_presents_absents($cid, $sid,$eid){
        $cours = Cour::findOrFail($cid);
        $seance = Seance::findOrFail($sid);

        $liste_etudiants = $cours->etudiants;
        $liste_presents = $seance->etudiants;
    ...
    public function gestionnaire_gestion_etudiants(){
    ...
    public function gestionnaire_create_etudiant_form(){
    ...
    public function gestionnaire_create_etudiant(Request $request){
        $request->validate([
            'nom' => 'required|string|min:1|max:40',
            'prenom' => 'required|string|min:1|max:40',
            'noet' => 'numeric|digits:8|nullable|unique:etudiants'
        ]);
        ...
        if($request->noet){
            ...
            $etudiant->noet = (string) rand(10000000,99999999);
        }
        //dd($etudiant->noet);
        $etudiant->save();
    ...
    public function gestionnaire_create_seance(Request $request, $id){
        $request->validate([
    ...
    public function gestionnaire_seance_de_ce_cours($id){
        $cours = Cour::findOrFail($id);
        $liste_seances = $cours->seances;
    ...
    public function gestionnaire_gestion_liste_enseignants(){
    ...
    public function gestionnaire_gestion_association_cours_enseignant($id){
        $liste_cours = Cour::all();
    ...
    public function gestionnaire_gestion_asso_association_cours_enseignant($eid, $cid){
        $cours = Cour::findOrFail($cid);
